...
public class RedirectUtils {
  ...
  public static String verifyRedirectUri(KeycloakSession session, String redirectUri,
      ClientModel client) {
    return verifyRedirectUri(session, redirectUri, client, true);
  }

  public static String verifyRedirectUri(KeycloakSession session, String redirectUri,
      ClientModel client, boolean requireRedirectUri) {
    if (client != null) {
      return verifyRedirectUri(session, client.getRootUrl(), redirectUri,
  ...
  private static Set<String> getValidateRedirectUris(KeycloakSession session) {
    ...
    return session.clientStorageManager().getAllRedirectUrisOfEnabledClients(realm).entrySet()
        .stream()
        .filter(me -> me.getKey().isEnabled() && OIDCLoginProtocol.LOGIN_PROTOCOL.equals(
            me.getKey().getProtocol()) && !me.getKey().isBearerOnly() && (
  ...
  public static String verifyRedirectUri(KeycloakSession session, String rootUrl,
      String redirectUri, Set<String> validRedirects, boolean requireRedirectUri) {
    ...
    if (redirectUri == null) {
      ...
      if (!valid && (r.startsWith(Constants.INSTALLED_APP_URL) || r.startsWith(
          Constants.INSTALLED_APP_LOOPBACK))
  ...
  private static String relativeToAbsoluteURI(KeycloakSession session, String rootUrl,
      String relative) {
    if (rootUrl != null) {
  ...
  private static boolean matchesRedirects(Set<String> validRedirects, String redirect) {
    for (String originalValidRedirect : validRedirects) {
      String validRedirect = originalValidRedirect;
      if (validRedirect.endsWith("*") && !validRedirect.contains("?")) {
        ...
        boolean isRegex = validRedirect.indexOf('*') > 0 || validRedirect.indexOf('?') > 0
            || validRedirect.indexOf('+') > 0;
        if (isRegex) {
          validRedirect = "^" + validRedirect; // avoiding matching query params
  ...
  public static String getFirstValidRedirectUri(KeycloakSession session, String rootUrl,
      Set<String> validRedirects) {
